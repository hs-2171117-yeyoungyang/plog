...
import { ChevronRight, Folder, Link, Database } from "lucide-react";
...
const ApiTree: React.FC<ApiTreeProps> = ({
  ...
  // 엔드포인트 우클릭 핸들러
  const handleEndpointContextMenu = (
    e: React.MouseEvent,
    endpointId: string,
    endpointPath: string
  ) => {
  ...
  const handleDelete = () => {
    ...
    switch (type) {
      ...
      case "group":
        ...
        deleteAction = () =>
          endpointIds && onDeleteGroup?.("", id.toString(), endpointIds);
        break;
    ...
    <div className={styles.apiTree}>
      {servers.map((server) => (
        <div key={server.id} className={styles.serverNode}>
          ...
          <div
            ...
            onContextMenu={(e) =>
              handleServerContextMenu(e, server.id, server.name, server.openapi_spec_id)
            }
          >
            <div className={styles.serverContent}>
              <div className={styles.itemWrapper}>
                ...
                <span
                  className={`CaptionLight ${styles.textContent}`}
                  title={server.name}
                >
                  {server.name}
                </span>
              ...
              <ChevronRight
                className={`${styles.chevron} ${
                  expandedServers.has(server.id) ? styles.chevronRotated : ""
                }`}
              />
            </div>
          </div>
          ...
          {/* 서버 그룹 리스트 */}
          <div
            className={`${styles.groupsContainer} ${styles.collapsible} ${
              expandedServers.has(server.id) ? styles.collapsibleOpen : ""
            }`}
          >
                ...
                <div key={group.id} className={styles.groupNode}>
                  ...
                  >
                    <div className={styles.groupContent}>
                      ...
                      <ChevronRight
                        className={`${styles.chevron} ${
                          expandedGroups.has(groupKey) ? styles.chevronRotated : ""
                        }`}
                      />
                    </div>
                  </div>
                  ...
                  {/* 그룹 엔드포인트 리스트 */}
                  <div
                    className={`${styles.endpointsContainer} ${styles.collapsible} ${
                      expandedGroups.has(groupKey) ? styles.collapsibleOpen : ""
                    }`}
                  >
                    {group.endpoints.map((endpoint) => (
                      <div
                        ...
                        onClick={() =>
                          handleEndpointClick(endpoint, server.name, group.name)
                        }
                        onContextMenu={(e) =>
                          handleEndpointContextMenu(e, endpoint.id, endpoint.path)
                        }
                      >
                        <div className={styles.endpointContent}>
                          <div className={styles.endpointWrapper}>
                            <Link
                              className={`${styles.endpointIcon} ${getMethodColor(
                                endpoint.method
                              )}`}
                            />
                            <span
      ...
      {contextMenu.visible && (
        <ActionMenu
          ...
          onVersionChange={
            contextMenu.type === "server" && contextMenu.openapi_spec_id
              ? handleVersionChange
              : undefined
          }
